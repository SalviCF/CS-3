...
% 
% (c) Juan Antonio Fernandez Madrigal, 2014-2016
...
classdef Scorbot < handle
    ...
    methods (Access = public)
        ...
        function obj = Scorbot(mode)
            ...
            if (obj.isvrepmode())
                ...
                if (obj.clientID>-1)
                    ...
                    % Define each jointHandler
                    obj.vrep.simxStartSimulation(obj.clientID,obj.vrep.simx_opmode_oneshot_wait);
                    [res,jointHandler0]=obj.vrep.simxGetObjectHandle(obj.clientID,'Scorbot_ER_V_PLUS_JOINT_0',obj.vrep.simx_opmode_oneshot_wait);
                    [res,jointHandler1]=obj.vrep.simxGetObjectHandle(obj.clientID,'Scorbot_ER_V_PLUS_JOINT_1',obj.vrep.simx_opmode_oneshot_wait);
                    [res,jointHandler2]=obj.vrep.simxGetObjectHandle(obj.clientID,'Scorbot_ER_V_PLUS_JOINT_2',obj.vrep.simx_opmode_oneshot_wait);
                    [res,pitchHandler]=obj.vrep.simxGetObjectHandle(obj.clientID,'Scorbot_ER_V_PLUS_JOINT_3',obj.vrep.simx_opmode_oneshot_wait);
                    [res,rollHandler]=obj.vrep.simxGetObjectHandle(obj.clientID,'Scorbot_ER_V_PLUS_JOINT_4',obj.vrep.simx_opmode_oneshot_wait);
                    
                    obj.joints = [jointHandler0 jointHandler1 jointHandler2 pitchHandler rollHandler];
                    
                    %obj.changeSpeed(60);
                    obj.scbspeed=60;
                    obj.consts=VREPscorbotConsts();
                    [thetas,pitch,roll,gripper]=VREProbotHome(obj.consts);
                    obj.state=struct('thetas',thetas,'pitch',pitch,'roll',roll,'gripper',gripper);
        ...
        % Move the robot to its homing position.
            ...
            if (obj.isvrepmode())
                [thetas,pitch,roll,gripper]=VREProbotHome(obj.consts);
                obj.changeGripper(gripper);
                obj.vrepJointsMove(thetas,pitch,roll);
                obj.state=struct('thetas',thetas,'pitch',pitch,'roll',roll,'gripper',gripper);  
            end
        ...
        %       changePos* methods of the Scorbot class.
            ...
            if (obj.isvrepmode())
                ...
                [uTs,Q,uTg,posetip]=VREPdirectModel(obj.consts,obj.state.thetas,obj.state.pitch,obj.state.roll);
                [motors,mpitch,mroll]=VREPthetasToMotors(obj.consts,obj.state.thetas,obj.state.pitch,obj.state.roll);
                pos=struct('joints',[round(motors) 0 0],...
                           'xyz',round(posetip(1:3)*1000*10),...
                           'pitch',mpitch,...
                           'roll',mroll);
        ...
        % NOTE: In simulation mode, WAIT==0 is not supported.
            ...
            if (obj.isrealmode())
                ...
                if (wait==1)||(wait==2)
                    finished=0;
                    while (~finished)
                        [r,curpos]=obj.currentPos();
                        t1=toc(t0);
                        if (r==0)
                            error('Unable to wait for finishing motion.');
                        end
                        if (wait==2)
                            t=[t t1];
                            p{length(p)+1}=curpos;
            ...
            if (obj.isvrepmode())
                ...
                [thetas1,pitch1,roll1]=VREPmotorsToThetas(obj.consts,pos.joints(1:3),pos.pitch,pos.roll);
                outofrange=VREPcheckThetasPR(obj.consts,thetas1,pitch1,roll1);
                if (outofrange)
                    error('Destination position unreachable.');
                end
                path=VREPdefinePath(obj.scbspeed/100*4*pi/180,...
                                   obj.state.thetas,obj.state.pitch,obj.state.roll,...
                                   thetas1,pitch1,roll1);
                n=length(path);
                t0=tic;
                for (f=1:n)
                    ...
                    obj.vrepJointsMove(obj.state.thetas,obj.state.pitch,obj.state.roll);
        ...
        %   PITCH <- New pitch value (tenths of degrees)
        ...
        % values of the joints (4,5), as it does in real mode.
            ...
            if (obj.isvrepmode())
                [thetas,p,r]=VREPmotorsToThetas(obj.consts,pos.joints(1:3),pitch,pos.roll);
                outofrange=VREPcheckThetasPR(obj.consts,thetas,p,r);
                if (outofrange)
                    error('Position out of range.\n');
                end
                newpos.pitch=pitch;
        ...
        %
        %   POS <- Position to be modified
        %   ROLL <- New roll value (tenths of degrees)
        %   NEWPOS -> Modified position
        %
        ...
        % values of the joints (4,5), as it does in real mode.
            ...
            if (obj.isvrepmode())
                [thetas,p,r]=VREPmotorsToThetas(obj.consts,pos.joints(1:3),pos.pitch,roll);
                outofrange=VREPcheckThetasPR(obj.consts,thetas,p,r);
                if (outofrange)
                    error('Position out of range.');
                end
        ...
        % instead).
            ...
            if (obj.isvrepmode())
                [motors,mpitch,mroll]=VREPthetasToMotors(obj.consts,obj.state.thetas,obj.state.pitch,obj.state.roll);
                [thetas,pitch,roll]=VREPmotorsToThetas(obj.consts,values(1:3),mpitch,mroll); % we leave pitch/roll unmodified
                outofrange=VREPcheckThetasPR(obj.consts,thetas,pitch,roll);
                if (outofrange)
                    error('Destination position is unreachable.');
                end
                [uTs,Q,uTg,posetip]=VREPdirectModel(obj.consts,thetas,pitch,roll);
                newpos.joints=values;
                newpos.xyz=round(posetip(1:3)*1000*10);
            end
        ...
        %
        %   POS <- Position to be modified
        %   XYZ <- New cartesian values for that position, in tenths of 
        %          millimeters
        %   NEWPOS -> Modified position
        %
        ...
        % Pitch and roll are not changed by this routine.
            ...
            if (obj.isvrepmode())
                [thetas,pitch,roll]=VREPmotorsToThetas(obj.consts,pos.joints(1:3),pos.pitch,pos.roll);
                outofrange=VREPcheckThetasPR(obj.consts,thetas,pitch,roll);
                if (outofrange)
                    error('Original position is out of range.');
                end
                [uTs,Q,uTg,posetip]=VREPdirectModel(obj.consts,thetas,pitch,roll);
                [nosols,thetas,pitch,roll]=VREPinverseModel(obj.consts,[xyz/10/1000 posetip(4:6)],thetas);
                if (nosols==0)
                    error('Destination position is unreachable.');
                end
                [motors,mpitch,mroll]=VREPthetasToMotors(obj.consts,thetas,pitch,roll); % no need to check outofrange because inverse model already does
                newpos.joints=[round(motors(1:3)) 0 0];
                newpos.xyz=xyz;
            end
        ...
        % the Scorbot and return when it is done.
            ...
            if (obj.isvrepmode())
                obj.vrep.simxSetIntegerSignal(obj.clientID,'RG2_open',openorclose,obj.vrep.simx_opmode_oneshot_wait);
                pause(2);
            end
        ...
        % NOTE: This method does nothing in real mode.
            ...
            if (obj.isvrepmode())
                res = VREPteachPendant(obj);
                if(~res)
                   error('An error has ocurred during the teachPendant task.');
                end
                lastpos = obj.currentPos();
            end
        ...
        function pendantMove(obj,joint,inc)
            if(joint<1 || joint>5)
               error('Invalid joint value');
            end
            ...
            switch(joint)
                ...
                case 5 % Roll
                    mroll = mroll + inc;
                otherwise % Motors
                    motors(joint) = motors(joint) + inc;
            end
            [thetas,pitch,roll]=VREPmotorsToThetas(obj.consts,motors,mpitch,mroll);
            newst.thetas=thetas;
            newst.pitch=pitch;
            newst.roll=roll;
        ...
        function pendantMoveCartesian(obj, axis, inc)
            [uTs,Q,uTg,posetip]=VREPdirectModel(obj.consts,obj.state.thetas,obj.state.pitch,obj.state.roll);
            newst=obj.state;
            newtip=posetip;
            newtip(axis) = newtip(axis) + inc;
            [nosols,thetas,pitch,roll]=VREPinverseModel(obj.consts,newtip,obj.state.thetas);
            if (nosols>0)
                newst.thetas=thetas;
                newst.pitch=pitch;
                newst.roll=roll;
    ...
    methods (Access = private)
        ...
        function checkmode(obj,m)
            ...
            switch (m)
                case {Scorbot.MODEREAL, Scorbot.MODESIM, Scorbot.MODEHYB, Scorbot.MODEVREP}
                otherwise
        ...
        function r=isvrepmode(obj,m)
            % return 1 if the object is in vrep modes
            if (nargin<2)
                m=obj.mode;
            end
            switch (m)
                case {Scorbot.MODEVREP}
                    r=1;
                otherwise
                    r=0;
            end
        ...
        function vrepJointsMove(obj,thetas,pitch,roll)
            alpha = pitch-(thetas(2)+thetas(3));% pitch universal joint.
            
            obj.vrep.simxSetJointTargetPosition(obj.clientID,obj.joints(1),thetas(1),obj.vrep.simx_opmode_oneshot);
            obj.vrep.simxSetJointTargetPosition(obj.clientID,obj.joints(2),thetas(2),obj.vrep.simx_opmode_oneshot);
            obj.vrep.simxSetJointTargetPosition(obj.clientID,obj.joints(3),thetas(3),obj.vrep.simx_opmode_oneshot);
            obj.vrep.simxSetJointTargetPosition(obj.clientID,obj.joints(4),alpha,obj.vrep.simx_opmode_oneshot);
            obj.vrep.simxSetJointTargetPosition(obj.clientID,obj.joints(5),roll,obj.vrep.simx_opmode_oneshot);
            [returnCode, position0] = obj.vrep.simxGetJointPosition(obj.clientID,obj.joints(1),obj.vrep.simx_opmode_oneshot_wait);
            [returnCode, position1] = obj.vrep.simxGetJointPosition(obj.clientID,obj.joints(2),obj.vrep.simx_opmode_oneshot_wait);
            [returnCode, position2] = obj.vrep.simxGetJointPosition(obj.clientID,obj.joints(3),obj.vrep.simx_opmode_oneshot_wait);
            [returnCode, position3] = obj.vrep.simxGetJointPosition(obj.clientID,obj.joints(4),obj.vrep.simx_opmode_oneshot_wait);
            [returnCode, position4] = obj.vrep.simxGetJointPosition(obj.clientID,obj.joints(5),obj.vrep.simx_opmode_oneshot_wait);

            while((abs(position0-thetas(1))>0.5 || abs(position1-thetas(2))>0.5 || abs(position2-thetas(3))>0.5 || abs(position3-alpha)>0.5 || abs(position4-roll)>0.5))                
                [returnCode, position0] = obj.vrep.simxGetJointPosition(obj.clientID,obj.joints(1),obj.vrep.simx_opmode_oneshot_wait);
                [returnCode, position1] = obj.vrep.simxGetJointPosition(obj.clientID,obj.joints(2),obj.vrep.simx_opmode_oneshot_wait);
                [returnCode, position2] = obj.vrep.simxGetJointPosition(obj.clientID,obj.joints(3),obj.vrep.simx_opmode_oneshot_wait);
                [returnCode, position3] = obj.vrep.simxGetJointPosition(obj.clientID,obj.joints(4),obj.vrep.simx_opmode_oneshot_wait);
