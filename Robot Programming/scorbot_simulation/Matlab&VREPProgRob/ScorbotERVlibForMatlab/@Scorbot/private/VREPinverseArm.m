function solutions=VREPinverseArm(scbconsts,Q,theta1)
...
% be obtained through VREPinverseGripper()), in THETA1 the angle of the base of
% the scorbot (also obtainable through VREPinverseGripper()), return in 
...
% the arm at Q, or an empty cell if no solution is found.
    ...
    t1T2=VREPlinkT(2,scbconsts,[theta1 0 0]); % we do not care here about any angle but theta(1); in particular, we only use the origin of the frame 2, not its orientation
    t0T1=VREPlinkT(1,scbconsts,[theta1 0 0]);
    tuT0=VREPlinkT(0,scbconsts,[theta1 0 0]);
    tuT2=tuT0*t0T1*t1T2;
    P=tuT2*[0;0;0;1];
    P=P(1:3); 
