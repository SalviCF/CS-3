function [nosols,thetas,pitch,roll]=VREPinverseModel(scbconsts,posetip,orientthetas)
% Return the best solution to set the gripper as in POSETIP (see
% VREPdirectModel()). ORIENTTHETAS may be empty (then this function chooses the
...
% PITCH,ROLL: the angles of the gripper (rads, w.r.t. the univ. frame)
    ...
    solsg=VREPinverseGripper(scbconsts,posetip);
    ng=length(solsg);
    if (ng==0)
        return;
    end
    ...
    solsa={};
    for (f=1:ng)
        solsf=VREPinverseArm(scbconsts,solsg{f}.Q,solsg{f}.theta1);
        nf=length(solsf);
        for (k=1:nf)
    ...
    for (f=1:dists)
        dists(f)=norm(solsa{f}.thetas,orienthetas);
    end
