...
bolsa2 :: Bag Char
bolsa2 = Node 'b' 3 (Node 'c' 2 (Node 'd'  1 Empty))
...
occurrences x (Node y oy s) | (x < y)   = 0   -- como se mantiene el orden, significa que no estará en el resto de la bolsa
                            | (x == y)  = oy
...
delete x (Node y oy s)  | (x < y)   = (Node y oy s)   -- x no estará en la bolsa puesto que se mantiene el orden
...
occurrences_insert_1 x s = occurrences x (insert x s) == 1 + occurrences x s   -- si x no estaba en s, ocurrencias = 1
...
{-
*DataStructures.Bag.SortedLinearBag> checkBag
+++ OK, passed 1 tests.
+++ OK, passed 100 tests.
+++ OK, passed 100 tests.
+++ OK, passed 100 tests.
...
union s Empty = s
union Empty s = s
union b1@(Node x ox s) b2@(Node y oy t)
    | (x < y)   = Node x ox (union s b2)
    | (x > y)   = Node y oy (union b1 t)
    | otherwise = Node x (ox+oy) (union s t)

intersection :: Ord a => Bag a -> Bag a -> Bag a
intersection s Empty = Empty
intersection Empty s = Empty
intersection b1@(Node x ox s) b2@(Node y oy t)
    | (x < y)   = intersection s b2    -- significa que x no está en la segunda bolsa
    | (x > y)   = intersection b1 t    -- significa que y no está en la primera bolsa
    | otherwise = Node x (min ox oy) (intersection s t)

difference :: Ord a => Bag a -> Bag a -> Bag a
difference s Empty = s
difference Empty s = Empty
difference b1@(Node x ox s) b2@(Node y oy t)
    | (x < y)   = Node x ox (difference s b2)    -- x no está en la segunda bolsa
    | (x > y)   = difference b1 t
    | otherwise = if (ox > oy) then Node x (ox - oy) (difference s t) else difference s t
...
{-
*DataStructures.Bag.SortedLinearBag> checkBagAux
+++ OK, passed 100 tests.
+++ OK, passed 100 tests.
+++ OK, passed 100 tests.
-}
...
La ordenada es algo mejor ya que el orden mejora la eficiencia promedio de las operaciones, aunque no la del peor caso.
...
{-
foldBag f z Bag
foldBag f [] Node 'a' 2 (Node 'b' 4 Empty)
f 'a' 2 (foldBag f [] Node 'b' 4 Empty)
f 'a' 2 (f 'b' 4 (foldBag f [] Empty)
f 'a' 2 (f 'b' 4 ([])
f 'a' 2 (['b'])
['a', 'b']
"ab"
