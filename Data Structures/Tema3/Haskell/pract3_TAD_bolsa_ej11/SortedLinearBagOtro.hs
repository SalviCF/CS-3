...
bolsa1 :: Bag Char
bolsa1 = Node 'a' 2 (Node 'b' 3 Empty)

bolsa2 :: Bag Char
bolsa2 = Node 'a' 5 (Node 'c' 1 Empty)
...
-- inserta un nuevo dato en una bolsa (oy = ocurrencias de y)
insert :: (Ord a) => a -> Bag a -> Bag a
...
delete x (Node y oy s) | x<y        = Node y oy s
                       | x==y       = if oy>1 then Node y (oy-1) s else s       -- Borro el nodo cuando las ocurrencias llegan a 0, no pongo Node x 0
...
union (Node x ox s) (Node y oy t)
      |x<y = (Node x ox (Node y oy (union s t)))    -- esta implementación mantiene nodos con el mismo elemento
...
union s     Empty = s
union Empty s     = s
union (Node x ox s) (Node y oy t) = union (delete x (Node x ox s)) (insert x (Node y oy t)) -- más costosa
...
difference (Node x ox s) (Node y oy t) | x == y = if ox>oy then Node x (ox-oy) (difference s t) else difference s (Node y oy t)
...
La representación ordenada, ya que puede mejorar la eficiencia promedio de las operaciones, aunque no la del peor caso.
...
keys :: Ord a => Bag a -> [a]
keys bag = foldBag (\x ox s -> x:s) [] bag
