...
-- Pepe Gallardo, 2012; merge function by Salvi CF
-------------------------------------------------------------------------------
...
merge h@(Node x sz lh rh) h'@(Node x' sz' lh' rh')
 | x < x'          = Node x (sz + sz') left right
 | otherwise       = merge h' h   -- fuerza a que el primer heap tenga la menor raíz
    where
      (a, b, c) = (h', lh, rh) --(ganador comparación, h izq perdedor, h derech perdedor)
      (left, right) | (maximum [size a, size b, size c] == size a) = (a, merge b c)
                    | (maximum [size a, size b, size c] == size b) = (b, merge a c)
                    | otherwise                                    = (c, merge a b)

-- left  = fst $ foldr1 (\(x,y) ac -> if (y > snd ac) then (x,y) else ac) [(a, size a), (b, size b), (c, size c)]
----------------------------------------------------------
-- ^^^^^^^^^^^^^^-- SOLO TOCAR ARRIBA ^^^^^^^^^^^ ---------
----------------------------------------------------------
